...
export default function Nav({show}) {
  ...
  const activeLink = inactiveLink+' bg-blue-300 text-white font-bold rounded';
  const logoutLinkActive = inactiveLink+' bg-red-300 text-white font-bold rounded';
  const inactiveIcon = 'w-6 h-6';
  const activeIcon = inactiveIcon + ' text-white font-bold';
  ...
  return (
    <aside className={(show?'left-0':'-left-full')+" top-0 text-gray-500 p-4 fixed w-full  h-full md:static md:w-auto transition-all"}>
      ...
      <nav className="flex flex-col mt-10 ml-8 gap-2">
        ...
        <button onClick={logout} className={logoutLinkActive}>
